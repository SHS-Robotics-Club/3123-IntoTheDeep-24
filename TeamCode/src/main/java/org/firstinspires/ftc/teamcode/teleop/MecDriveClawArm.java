...
/*
        ...
        if (Math.abs(gp2LeftJoystickY) > 0.1) { // Dead zone to avoid accidental movement
            robot.getClawArm().setManualPower(gp2LeftJoystickY);
        } else if (gamepad2.a) {
            robot.getClawArm().setTargetPosition(90.0); // Move arm to 90 degrees
        } else if (gamepad2.b) {
            robot.getClawArm().setTargetPosition(10.0); // Move arm to lower stop
        }
